...
public class DriverDownloaderPanel extends JPanel {
			...
			public void mouseClicked(MouseEvent arg0) {
				...
				if (arg0.getClickCount()==2) {
					try {
			    		...
			    		String url = driverManager.getURL(name);
			    		openUrl(url, name);
					} catch (Exception ex) {
    ...
    private void selectedDownloadsTableMouseClicked(java.awt.event.MouseEvent evt) {
		...
		int row = selectedDownloadsTable.getSelectedRow();
        if (2==col) {
    		String url = selectedDownloads.get(row).url;
	...
	private void openUrl(String url, String model) throws HeadlessException {
    ...
    void showModelUrl() {
    	try {
			String name = (String)modelsList.getSelectedValue();
    		String url = driverManager.getURL(name);
    		JOptionPane.showInputDialog(this, "URL for "+name, url);
    	} catch (Exception ex) {
    ...
    public void addSelectedModels() {
        ...
        for (int idx: selection) {
            ...
            if (null!=directory) {
            	try {
	                String url = driverManager.getURL(model);
