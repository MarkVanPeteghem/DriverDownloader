...
/**
 * Class to download drivers from EMachines
 * Note: EMachines is affiliated in some way with Acer,
 *       because the main page for the drivers is on acer.com,
 *       and the structure of the site is exactly the same,
 *       but the models are different.
 *       The website contains some forms that use the POST method to
 *       submit, which makes it a little harder to scrape the pages.
 * @author Mark Van Peteghem
 *
 */
public class EMachinesDriverDownloader extends DriverDownloader {

	// note that SC=PA_6E& has to be included here, otherwise we get a page without subcategories
	private final String homepage = "http://global-download.acer.com/GDHome.aspx?BC=eMachines&SC=PA_6E&LC=en";
	...
	protected Map<String, PostableUrl> getCategories() throws Exception {
		...
		for (String input: inputs) {
			if (!input.contains("__VIEWSTATE")) {
				...
				String categoryLink = "http://global-download.acer.com/Step2.aspx?Step1="+URLEncoder.encode(name, "UTF-8")+"&Step2=&Step3=&BC=eMachines&SC=PA_6E&LC=en";
	...
	protected void updateCategory(DownloadedPage page) throws Exception {
		// remove SC=PA_6E& to get all the models
		String generalLink = page.getUrl().replace("Step2.aspx", "Step3.aspx").replace("SC=PA_6E&", "");
		...
		for (String name: names) {
			...
			DownloadedPage catPage = download.getPage(new PostableUrl(link));
            if (Interrupt.get()) {
                return;
            }
			updateProductLine(catPage);
		}
	}
